...
const writeMetaXml = async ({hasEmof, eventCore, occExt, dnaExt, path, ignoreHeaderLines} ) =>  await fs.promises.writeFile(`${path}/archive/meta.xml`, util.metaXmlEventCore(eventCore, occExt, dnaExt, hasEmof, ignoreHeaderLines))

const getDefaultTermsForMetaXml = (biomData, dnaTerms, occTerms) => {
  let occDefaultTerms = []
  let dnaDefaultTerms = [] 
  let eventDefaultTerms = []
      ...
      if(parsed?.defaultValues?.sample){
        Object.keys(parsed?.defaultValues?.sample).forEach(key => {
          ...
          if(dnaTerms.has(key)){
            ...
            eventDefaultTerms.push({...occTerms.get(key), default: parsed?.defaultValues?.sample[key]})
          }
        })
      }
    } catch (error) {
  ...
  return {occDefaultTerms, dnaDefaultTerms,eventDefaultTerms, keySet}
}
...
const writeEmofForRow = async (emofStream, termMapping, sample) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    Object.keys(measurements).forEach(m => { 
      ...
      if (hasValue) {
        dataString += `${sample.id}\t${measurements[m]?.measurementType || ""}\t${sample?.metadata?.[m]}\t${otherEMOFfields.map(f => measurements[m]?.[f] || "").join("\t")}\n`
      }
  ...
  return new Promise(async (resolve, reject) => {
    try{
     ...
     // const reverseTaxonTerm = key =>  key //_.get(reverseTaxonTerms, `${key}`, key);
      ...
      const eventTerms = await util.dwcTerms('dwc_event');
    ...
    // Make a set of sampleHeaders to avoid terms exist in both taxon and sample 
      ...
      const relevantEventTerms = [...sampleHeaders.filter(key => eventTerms.has(key)).map(key => eventTerms.get(key))]
     ...
     console.log("Taxon headers: " +taxonHeaders)
      ...
      const occCoreTerms = [...relevantOccTerms, occTerms.get('sampleSizeValue'), occTerms.get('sampleSizeUnit'), occTerms.get('organismQuantity'), occTerms.get('organismQuantityType'), occTerms.get('basisOfRecord')]
      console.log("OCC terms: "+ occCoreTerms.map(k => k.name))

      await writeMetaXml({hasEmof, eventCore: [...relevantEventTerms, ...defaults.eventDefaultTerms], occExt: [...relevantOccTerms, ...defaults.occDefaultTerms], dnaExt: [...relevantDnaTerms, ...defaults.dnaDefaultTerms], path, ignoreHeaderLines})

      
      const eventStream = fs.createWriteStream(`${path}/archive/event.txt`, {
          flags: "a",
        });
        if(ignoreHeaderLines === 1){
          eventStream.write(`eventID\t${relevantEventTerms.map(k => k.name).join("\t")}\n`)
        } 
      const occStream = fs.createWriteStream(`${path}/archive/occurrence.txt`, {
          flags: "a",
        });
        if(ignoreHeaderLines === 1){
          occStream.write(`eventID\toccurrenceID\t${occCoreTerms.map(k => k.name).join("\t")}\n`)
        }
      const dnaStream = fs.createWriteStream(`${path}/archive/dna.txt`, {
          flags: "a",
        });
      if(ignoreHeaderLines === 1){
          dnaStream.write(`eventID\toccurrenceID\t${relevantDnaTerms.map(k => k.name).join("\t")}\n`)
        }
      const emofStream = hasEmof ? fs.createWriteStream(`${path}/archive/emof.txt`, {
          flags: "a",
        }) : null;
        if(hasEmof && ignoreHeaderLines === 1){
          emofStream.write(`eventID\t${["measurementType", "measurementValue", ...otherEMOFfields ].join("\t")}\n`)
        }
      ...
      let emofStreamClosed = hasEmof ? false : true;
       ...
       const allStreamsClosed = () => {
            return eventStreamClosed && occStreamClosed && dnaStreamClosed && emofStreamClosed;
        }
      ...
      const getDataForTermFromTaxon = (taxon, terms) => terms.filter(term => !sampleHeaderSet.has(term.name) && term.name in taxon.metadata).map(term => taxon.metadata[term.name] || "").join("\t"); 
           ...
           for await (const [cidx, c] of biomData.columns.entries()) {
            ...
            const eventData = getDataForTermfromSample(c, relevantEventTerms)
            if(eventData){
              if(!eventStream.write(`${c.id}\t${eventData}\n`)){
                await once(eventStream, 'drain');
              };
            }

            if (hasEmof){
                await writeEmofForRow(emofStream, termMapping, c, c.id);
                }
            ...
            for await (const [i, r] of rowData.entries()) {
                if (Number(r) > 0) {
                   ...
                   if(!occStream.write(`${sampleId}\t${occurrenceId}\t${occSampleData ? `${occSampleData}\t` : ""}${occTaxonData ? `${occTaxonData}\t` : ""}${_.get(c, 'metadata.readCount','')}\t${DEFAULT_UNIT}\t${r}\t${DEFAULT_UNIT}\t${BASIS_OF_RECORD}\n`)){
                    ...
                    if(!dnaStream.write(`${c.id}\t${occurrenceId}\t${dnaSampleData ? `${dnaSampleData}\t` : ""}${dnaTaxonData ? dnaTaxonData : ""}\n`)){
...
/* export const otuTableToDWC = async (otuTableFile, sampleFile, taxaFile, termMapping, path, delimiter) => {
  ...
  try{
    ...
    const samples = await streamReader.readMetaDataAsMap(sampleFile, ()=>{}, _.get(termMapping, 'samples', {}));
    const taxa = await streamReader.readMetaDataAsMap(taxaFile, ()=>{}, _.get(termMapping, 'taxa', {}));
    ...
    await writeMetaXml({hasEmof, occCore: [...relevantOccTerms, occTerms.get('sampleSizeValue'), occTerms.get('sampleSizeUnit'), occTerms.get('organismQuantity'), occTerms.get('organismQuantityType'),], dnaExt:relevantDnaTerms})
