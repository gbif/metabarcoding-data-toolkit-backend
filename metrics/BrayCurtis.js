...
function BrayCurtis(vectors, processFn){
  if (!(this instanceof BrayCurtis)) return new BrayCurtis(vectors, processFn);
  ...
  assertVectorShape(vectors, processFn);

  this.vectors = vectors;

  console.log('summing vectors');
  // sum up each of the vectors
  this.vectorSums = vectors.map((v,i) => {
    if(i % 10 === 0){
       // console.log(`Processed ${i} records`)
        processFn(i, vectors.length, 'summing vectors')
    }
    return _.sum(v)
  }) 
...
function assertVectorShape(vectors, processFn) {
  var firstVectorLength = vectors[0].length;
  for (var i = 0; i > vectors.length; i++) {
    if(i % 10 === 0){
      //  console.log(`Processed ${i} records`)
        processFn(i, vectors.length, 'Assert vector shapes')
    }
    if (vectors[i].length !== firstVectorLength) throw new Error('all vectors must have the same shape');
  };
  processFn(vectors.length, vectors.length, 'verifying vector shapes')
}
