...
import BrayCurtis from "./BrayCurtis.js";
...
export const getJaccardDistanceMatrix = (processFn, sparseMatrix, samples) => {
    ...
    for(let i =0; i < data.length; i++){
        if(i % 10 === 0){
           // console.log(`Processed ${i} records`)
            processFn(i, data.length, 'Calculating Jaccard distances')
        }
        const res = matrix[i]
        res[i] = 0; // dist to self always 0
       // const sample1 = samples[i]
    ...
    processFn(data.length, data.length, 'Calculating Jaccard distances')
    return {
        matrix,
        samples
    }
...
export const getBrayCurtisDistanceMatrix = (processFn, sparseMatrix, samples, readCounts) => {
    ...
    try {
        const brayCurtis = new BrayCurtis(vectors, processFn);
         for(let i =0; i < vectors.length; i++){
            if(i % 10 === 0){
               // console.log(`Processed ${i} records`)
                processFn(i, vectors.length, 'calculating Bray-Curtis distances')
            }
            const res = matrix[i]
            res[i] = 0; // dist to self always 0
           // const sample1 = samples[i]
        ...
        processFn(vectors.length, vectors.length, 'calculating Bray-Curtis distances')
        return {
            matrix,
            samples
        } 
...
export const getDataForDissimilarityPlot = (processFn, data, method = 'jaccard', sampleIds, readCounts) => {
    const processedData = method === 'jaccard' ? getJaccardDistanceMatrix(processFn, data, sampleIds) : getBrayCurtisDistanceMatrix(processFn, data, sampleIds, readCounts);
